...
CREATE TABLE
    user_company (
        ...
        hire_date DATE, -- PTO/Time off fields
        ...
        pto_accrual_rate DECIMAL(5, 2) NOT NULL DEFAULT 0.0, -- hours per pay period
...
CREATE TABLE
    invite_tokens (
        ...
        team_id UUID, -- Will reference teams table (created later)
        expires_at TIMESTAMPTZ NOT NULL,
        used_at TIMESTAMPTZ,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW ()
    );
...
CREATE TABLE
    pto_balance_history (
        ...
        user_id UUID NOT NULL REFERENCES users (id) ON DELETE CASCADE,
        company_id UUID NOT NULL REFERENCES companies (id) ON DELETE CASCADE,
        balance_type VARCHAR(20) NOT NULL, -- 'pto', 'sick', 'personal'
        change_type VARCHAR(20) NOT NULL, -- 'accrual', 'usage', 'adjustment'
        hours_changed INTEGER NOT NULL,
        previous_balance INTEGER NOT NULL,
        new_balance INTEGER NOT NULL,
        description TEXT,
        related_time_off_id UUID, -- Will reference time_off_requests table
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW ()
    );
...
CREATE TABLE
    company_activities (
        ...
        activity_type VARCHAR(100) NOT NULL, -- 'create', 'update', 'delete', etc.
        entity_type VARCHAR(100) NOT NULL, -- 'user', 'shift', 'time_off', etc.
        ...
        ip_address TEXT, -- IP address of the user performing the action
        user_agent TEXT, -- User agent string of the request
