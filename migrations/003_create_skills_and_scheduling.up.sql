...
CREATE TABLE
    skills (
        ...
        description TEXT,
        category VARCHAR(50) NOT NULL DEFAULT 'general', -- 'general', 'certification', 'equipment', 'management'
...
CREATE TABLE
    user_skills (
        ...
        verified_by UUID REFERENCES users (id) ON DELETE SET NULL, -- Manager who verified
        notes TEXT,
...
CREATE TABLE
    shift_required_skills (
        ...
        skill_id UUID NOT NULL REFERENCES skills (id) ON DELETE CASCADE,
        is_required BOOLEAN NOT NULL DEFAULT TRUE, -- TRUE = required, FALSE = preferred
...
CREATE TABLE
    shift_assignments (
        ...
        assignment_status VARCHAR(50) NOT NULL DEFAULT 'pending', -- 'pending', 'accepted', 'declined', 'cancelled', 'expired'
        acceptance_deadline TIMESTAMPTZ,
        response VARCHAR(50), -- 'accept', 'decline'
        response_notes TEXT,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW (),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW (),
...
CREATE TABLE
    shift_swap_responses (
        ...
        response_type VARCHAR(50) NOT NULL, -- 'interested', 'accepted', 'declined'
        notes TEXT,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW (),
