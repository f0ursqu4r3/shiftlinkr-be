...
CREATE INDEX IF NOT EXISTS idx_time_off_requests_end_date ON time_off_requests(end_date);
...
CREATE TABLE IF NOT EXISTS shift_swaps (
    ...
    SET
        NULL,
        FOREIGN KEY (target_shift_id) REFERENCES shifts(id) ON DELETE
    SET
        NULL,
        FOREIGN KEY (approved_by) REFERENCES users(id) ON DELETE
    SET
...
-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_shift_swaps_requesting_user_id ON shift_swaps(requesting_user_id);

CREATE INDEX IF NOT EXISTS idx_shift_swaps_original_shift_id ON shift_swaps(original_shift_id);

CREATE INDEX IF NOT EXISTS idx_shift_swaps_target_user_id ON shift_swaps(target_user_id);

CREATE INDEX IF NOT EXISTS idx_shift_swaps_target_shift_id ON shift_swaps(target_shift_id);
