...
CREATE TABLE
    shifts (
        ...
        STATUS VARCHAR(50) NOT NULL DEFAULT 'open', -- 'open', 'assigned', 'completed', 'cancelled'
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW (),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW ()
    );
...
CREATE TABLE
    shift_swaps (
        ...
        notes TEXT,
        swap_type VARCHAR(50) NOT NULL, -- 'open', 'targeted'
        STATUS VARCHAR(50) NOT NULL DEFAULT 'open', -- 'open', 'pending', 'approved', 'denied', 'completed', 'cancelled'
        approved_by UUID REFERENCES users (id) ON DELETE SET NULL,
        approval_notes TEXT,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW (),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW ()
...
ALTER TABLE invite_tokens ADD CONSTRAINT fk_invite_tokens_team_id FOREIGN KEY (team_id) REFERENCES teams (id) ON DELETE SET NULL;

-- Add foreign key reference for related_time_off_id in pto_balance_history
ALTER TABLE pto_balance_history ADD CONSTRAINT fk_pto_balance_history_time_off_id FOREIGN KEY (related_time_off_id) REFERENCES time_off_requests (id) ON DELETE SET NULL;
