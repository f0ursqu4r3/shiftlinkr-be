...
async fn test_get_time_off_stats_success() {
    ...
    create_test_time_off_request_with_status(&test_app.db.pool, &user.id, TimeOffStatus::Pending)
        .await;
    create_test_time_off_request_with_status(&test_app.db.pool, &user.id, TimeOffStatus::Approved)
        .await;
    create_test_time_off_request_with_status(&test_app.db.pool, &user.id, TimeOffStatus::Denied)
        .await;
...
async fn create_test_shift_with_status(
    pool: &SqlitePool,
    location_id: i64,
    assigned_user_id: Option<String>,
    status: ShiftStatus,
...
async fn create_test_shift_for_date(
    pool: &SqlitePool,
    location_id: i64,
    assigned_user_id: Option<String>,
...
async fn create_test_time_off_request_with_status(
    pool: &SqlitePool,
    user_id: &str,
    ...
    let query =
        "UPDATE time_off_requests SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
    sqlx::query(query)
        .bind(status.to_string())
        .bind(request.id)
        .execute(pool)
