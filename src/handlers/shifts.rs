...
use crate::database::models::{
    AssignmentResponse, ShiftAssignmentInput, ShiftClaimInput, ShiftInput, ShiftQuery,
    ShiftQueryType, ShiftStatus,
};
...
pub async fn get_shifts(
    ...
    let shifts = shift_repo
        .find_by_query(query.into_inner())
        .await
        .map_err(|e| {
...
pub async fn update_shift(
    ...
    let updated_shift = shift_repo
        .update_shift(shift_id, input.into_inner())
        .await
        .map_err(|e| {
...
pub async fn assign_shift(
    ...
    let shift = shift_repo
        .assign_shift(shift_id, assigned_user_id)
        .await
        .map_err(|e| {
    ...
    let mut metadata = ActivityLogger::metadata(vec![
...
pub async fn unassign_shift(
    ...
    let mut metadata =
        ActivityLogger::metadata(vec![(&"location_id", shift.location_id.to_string())]);
...
pub async fn update_shift_status(
    ...
    let shift = shift_repo
        .update_shift_status(shift_id, status.clone())
        .await
        .map_err(|e| {
            log::error!("Failed to fetch shift {}: {}", shift_id, e);
            AppError::DatabaseError(e)
...
pub async fn delete_shift(
    ...
    let metadata = ActivityLogger::metadata(vec![("shift_id", shift_id.to_string())]);
...
pub async fn respond_to_assignment(
    ...
    // Parse response
    let response = response_data.response.clone();
    ...
    if is_accepted {
        shift_repo
            .update_shift_status(assignment.shift_id, ShiftStatus::Assigned)
            .await
            .map_err(|e| {
...
pub async fn claim_shift(
    ...
    let claim = shift_claim_repo
        .create_claim(&claim_input)
        .await
        .map_err(|e| {
            log::error!(
...
pub async fn get_shift_claims(
    ...
    let claims = shift_claim_repo
        .get_claims_by_shift(shift_id)
        .await
        .map_err(|e| {
...
pub async fn approve_shift_claim(
    ...
    let approved_claim = shift_claim_repo
        ...
        .ok_or_else(|| AppError::NotFound("Claim not found or already processed".to_string()))?;

    // Assign the shift to the user
    let assigned_shift = shift_repo
        .assign_shift(claim.shift_id, claim.user_id)
        .await
        .map_err(|e| {
            log::error!(
