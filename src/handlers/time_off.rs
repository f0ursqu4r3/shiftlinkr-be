...
pub async fn create_time_off_request(
    ...
    let time_off_request = time_off_repo
...
pub async fn get_time_off_requests(
    ...
    let time_off_requests = time_off_repo
        .get_requests(Some(target_user_id), status_filter, start_date, end_date)
        .await
        .map_err(|e| {
...
pub async fn update_time_off_request(
    ...
    let updated_request = time_off_repo
...
pub async fn delete_time_off_request(
    ...
    let company_id = user_context.strict_company_id()?;
    let metadata = ActivityLogger::metadata(vec![("target_user", target_user_id.to_string())]);
    if let Err(e) = activity_logger
        .log_time_off_activity(
            company_id,
            Some(user_context.user_id()),
...
pub async fn approve_time_off_request(
    ...
    let approved_request = time_off_repo
...
pub async fn deny_time_off_request(
    ...
    let denied_request = time_off_repo
