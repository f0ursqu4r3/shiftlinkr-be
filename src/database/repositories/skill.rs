...
impl SkillRepository {
    ...
    pub async fn get_skill_by_id(&self, id: i64) -> Result<Option<Skill>> {
        let skill = sqlx::query_as::<_, Skill>(
            "SELECT id, name, description, created_at, updated_at FROM skills WHERE id = $1",
        )
        .bind(id)
    ...
    pub async fn update_skill(&self, id: i64, input: SkillInput) -> Result<Option<Skill>> {
        ...
        let skill = sqlx::query_as::<_, Skill>(
            r#"
            UPDATE skills SET name = $1, description = $2, updated_at = $3
            WHERE id = $4
            RETURNING id, name, description, created_at, updated_at
            "#,
        )
    ...
    pub async fn get_user_skills(&self, user_id: &str) -> Result<Vec<UserSkill>> {
        let user_skills = sqlx::query_as::<_, UserSkill>(
            "SELECT id, user_id, skill_id, proficiency_level, created_at, updated_at FROM user_skills WHERE user_id = $1"
        )
        .bind(user_id)
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn update_user_skill(
        ...
        let user_skill = sqlx::query_as::<_, UserSkill>(
            r#"
            UPDATE user_skills SET proficiency_level = $1, updated_at = $2
            WHERE id = $3
            RETURNING id, user_id, skill_id, proficiency_level, created_at, updated_at
            "#,
        )
    ...
    pub async fn get_shift_required_skills(
        ...
        let shift_skills = sqlx::query_as::<_, ShiftRequiredSkill>(
            "SELECT id, shift_id, skill_id, required_level, created_at FROM shift_required_skills WHERE shift_id = $1"
        )
        .bind(shift_id)
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn remove_shift_required_skill(&self, shift_id: i64, skill_id: i64) -> Result<bool> {
        let result =
            sqlx::query("DELETE FROM shift_required_skills WHERE shift_id = $1 AND skill_id = $2")
                .bind(shift_id)
                .bind(skill_id)
                .execute(&self.pool)
                .await?;
        ...
        let query = if let Some(level) = min_level {
            match level {
                ProficiencyLevel::Expert => {
                    "SELECT DISTINCT user_id FROM user_skills WHERE skill_id = $1 AND proficiency_level = 'expert'"
                },
                ProficiencyLevel::Advanced => {
                    "SELECT DISTINCT user_id FROM user_skills WHERE skill_id = $1 AND proficiency_level IN ('advanced', 'expert')"
                },
                ProficiencyLevel::Intermediate => {
                    "SELECT DISTINCT user_id FROM user_skills WHERE skill_id = $1 AND proficiency_level IN ('intermediate', 'advanced', 'expert')"
                },
                ProficiencyLevel::Beginner => {
                    "SELECT DISTINCT user_id FROM user_skills WHERE skill_id = $1 AND proficiency_level IN ('beginner', 'intermediate', 'advanced', 'expert')"
                }
            }
        } else {
            "SELECT DISTINCT user_id FROM user_skills WHERE skill_id = $1"
        };
